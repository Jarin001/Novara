 ...
 */
async function upsertAuthor(supabase, name, affiliation = null) {
  // Find existing author by name - use limit(1) to handle duplicates safely
  ...
  if (existing) {
    ...
    if (affiliation && existing.affiliation !== affiliation) {
      ...
      if (updateError) throw updateError;
      return (updatedRows && updatedRows.length > 0) ? updatedRows[0] : existing;
    }
    return existing;
  }
 ...
 */
async function getPaperWithAuthors(supabase, paperId) {
  const { data: rows, error: paperError } = await supabase
    .from('papers')
    .select('*')
    .eq('id', paperId)
    .limit(1);
  ...
  const paper = rows && rows.length > 0 ? rows[0] : null;
