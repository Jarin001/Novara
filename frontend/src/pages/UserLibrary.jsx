      ...
      for (const user of selectedUsers) {
        try {
          ...
          if (!response.ok) {
            ...
            // Check if it's an "already has access" error
            if (errorData.message && errorData.message.includes("already")) {
              skippedCount++;
            } else {
              errors.push(`${user.name}: ${errorData.message}`);
            }
          } else {
            successCount++;
          }
        } catch (userErr) {
          errors.push(`${user.name}: ${userErr.message}`);
        }
      }

      // Show success message if at least one user was shared
      if (successCount > 0) {
        let message = `Library shared with ${successCount} user${successCount > 1 ? "s" : ""}`;
        if (skippedCount > 0) {
          message += ` (${skippedCount} already had access)`;
        }
        setSuccess(message);
        setTimeout(() => {
          const successfulUsers = selectedUsers.slice(0, successCount);
          onShare(successfulUsers);
          handleCloseModal();
        }, 1500);
      } else if (skippedCount === selectedUsers.length) {
        // All users already had access
        setSuccess("All selected users already have access to this library");
        setTimeout(() => {
          handleCloseModal();
        }, 1500);
      } else {
        // Some errors occurred
        throw new Error(errors.join(", ") || "Failed to share library");
      }
    } catch (err) {
