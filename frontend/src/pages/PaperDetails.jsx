...
const PaperDetails = () => {
  ...
  // MODIFIED FUNCTION: Send message to AI backend
  const handleSendMessage = async () => {
    if (!inputValue.trim() || !paper || isSending || !pdfAvailable || !pdfUrl) return;
  ...
  return (
    <>
      ...
      {!chatOpen && !paperLoading && !paperError && (
        <button
          ...
          style={{
            ...
            height: 48,
            borderRadius: 8,
            background: pdfAvailable ? 'rgb(9, 75, 42)' : '#cccccc',
            color: '#fff',
            border: 'none',
            ...
            alignItems: 'center',
            justifyContent: 'center',
            boxShadow: pdfAvailable ? '0 4px 12px rgba(62, 81, 62, 0.4)' : 'none',
            zIndex: 999,
            transition: 'all 0.3s ease',
            letterSpacing: '0.5px',
      ...
      <div style={{
        ...
        {/* Messages Area - Show PDF warning if no PDF available */}
        <div style={{
          flex: 1,
          overflowY: 'auto',
          padding: '16px',
          ...
          {/* Show PDF warning at the top if PDF is not available */}
          {!pdfAvailable && pdfUrl && (
            <div style={{
              padding: '12px 16px',
              background: '#f8f9fa',
          ...
          {!pdfAvailable && !pdfUrl && (
            <div style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 500, color: '#c0392b', marginBottom: 4 }}>
                ⚠️ No PDF Available
              </div>
              <div style={{ fontSize: 12, color: '#666', lineHeight: 1.4 }}>
        ...
        <div style={{
          ...
          <input
            type="text"
            placeholder={pdfAvailable ? "Ask about the paper..." : "PDF not available for chat"}
            value={inputValue}
            onChange={(e) => pdfAvailable && setInputValue(e.target.value)}
            onKeyPress={(e) => pdfAvailable && e.key === 'Enter' && !isSending && handleSendMessage()}
            disabled={isSending || !paper || !pdfAvailable}
            style={{
              flex: 1,
              padding: '8px 12px',
              border: '1px solid #ddd',
              ...
              fontSize: 13,
              outline: 'none',
              opacity: isSending || !paper || !pdfAvailable ? 0.7 : 1,
              cursor: isSending || !paper || !pdfAvailable ? 'not-allowed' : 'text',
              background: !pdfAvailable ? '#f5f5f5' : '#fff'
            }}
          />
          <button
            onClick={handleSendMessage}
            disabled={!inputValue.trim() || isSending || !paper || !pdfAvailable}
            style={{
              padding: '8px 12px',
              background: (!inputValue.trim() || isSending || !paper || !pdfAvailable) ? '#cccccc' : '#3E513E',
              color: '#fff',
              border: 'none',
              borderRadius: 4,
              cursor: (!inputValue.trim() || isSending || !paper || !pdfAvailable) ? 'not-allowed' : 'pointer',
              fontSize: 13,
              fontWeight: 500,
              minWidth: 60
            }}
