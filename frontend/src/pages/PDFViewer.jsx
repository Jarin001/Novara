...
const PDFViewer = () => {
  ...
  const renderAnnotations = () =>
    annotations.filter(a => a.pageNumber === pageNumber && !a.isDeleted).map(ann => {
      ...
      if (ann.type === 'highlight') {
        ...
        return (
          <div key={ann._id}
            onClick={e => { e.stopPropagation(); if (isOwner) { setSelectedAnnotation(ann); setSidebarNoteId(null); } }}
            onMouseEnter={() => setHoveredAnnotationId(ann._id)}
            onMouseLeave={() => setHoveredAnnotationId(null)}
            style={{ position: 'absolute', left: `${x*100}%`, top: `${y*100}%`, width: `${width*100}%`, height: `${height*100}%`, backgroundColor: ann.color || '#FFFF00', opacity: isHovered ? 0.65 : 0.45, mixBlendMode: 'multiply', cursor: isOwner ? 'pointer' : 'default', pointerEvents: 'auto', transition: 'opacity 0.15s' }}
          >
  ...
  const renderHighlightPopup = () => {
    ...
    if (selectedAnnotation.userId !== currentUserId) return null; // safety: never show for others' highlights
